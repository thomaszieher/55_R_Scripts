...
genuchtenFit=nls(vol~vanGenuchten(potential,theta_r,theta_s,alpha,n),
        data=data.fit,
        start=startvalues,
        algorithm="port",
...
psi=seq(0,5,by=0.2)
wc=vanGenuchten(psi,coef(genuchtenFit)[1],coef(genuchtenFit)[2],coef(genuchtenFit)[3],coef(genuchtenFit)[4])
...
##calculate hydraulic diffusivity (van Genuchten 1980)
diffus=function(Ks,theta,theta_r,theta_s,alpha,n){
  m=1-1/n
  diff=((1-m)*Ks)/(alpha*m*(theta_s-theta_r))*
    theta^(0.5-1/m)*
    (((1-theta^(1/m))^(-m))+
       ((1-theta^(1/m))^m)-2)
  return(diff)
...
plot(seq(0.01,0.99,0.01),
     diffus(0.00000025,seq(0.01,0.99,0.01),coef(genuchtenFit)[1],coef(genuchtenFit)[2],coef(genuchtenFit)[3],coef(genuchtenFit)[4]),
     #ylim=c(-0.000001,0),
     xlim=c(0,1),
     type="l",
     xlab="saturation",
...
plot(seq(0.01,0.99,0.01),
     ktheta(seq(0.01,0.99,0.01),coef(genuchtenFit)[4]),
     #ylim=c(-0.000001,0),
     type="l",
     xlab="saturation",
...
corelList[corelList$cor==max(corelList$cor),]
...
mtext(side=2,"Depth [m]",line=2.5,las=0)
mtext(side=4,line=4,"Water content [%]",las=0)
plot(RatioDatarast-data.r,col=rev(colors),add=T,zlim=c(-6,0))

#require(rgl)
#plot3d(log10(corelList$ks),log10(corelList$ss),corelList$cor)
```
