...
rootPath="C:\\PHD\\Code_Projects\\55_check_ert\\01_DATA"
grassPath="C:\\PHD\\Code_Projects\\55_check_ert\\01_DATA\\grass"
setwd(rootPath)
...
getLogDiff=function(rastT1,rastT2){
  diff=log10(t(as.matrix(rastT2))-t(as.matrix(rastT1)))
  raster(diff)
}
getLog=function(rast){
  rast.log=rast
  rast.log@data=log10(rast@data)
  as.Raster(rast.log)
}
...
###Overall changes
Logarithm (base 10) of range of all datasets
```{r echo=F,message=F,warning=F}
##############################################################################
##r.series maps
##############################################################################
...
#execGRASS("r.series",input=rlist[2:length(rlist)],output="total_range",method="range",flag="overwrite")

##EXPLORE r.series raster
rastName="total_range"##"total_sum"
rast.series=as.Raster(readRAST6(rastName))
##plot range-raster
rast.series.log=rast.series
rast.series.log@data@values=log10(rast.series@data@values)
par(xaxs="i",yaxs="i")
plot(rast.series.log,col=colors)
points(dcpt.vect,pch=20,cex=0.5)
points(probesERT,pch=3,cex=0.5)
mtext(side=4,line=4.5,"log10(value range)",las=0)
```
...
plot(rangeERT,main="")
text(side=4,line=4,"Range [Ohm m]")
...
mtext(side=1,"Mean error [Ohm m]",line=2.5)
...
for (i in seq(start,end,by=1)){
  ...
  #rast2=as.Raster(readRAST6(rlist[i+1]))
  #uncertaintyDiff=data.frame(uncert=((((rast2+rangeERT/2)-(rast1-rangeERT/2)))/rangeERT)@data@values)
  uncertaintyDiff=data.frame(uncert=(rast1-material.before)@data@values)
...
###Build depth mask
Restrict further analysis to upper 2m within irrigation plot
```{r echo=F,message=F,warning=F}
...
#plot(NA,NA,ylim=c(depthMax,0),xlim=c(-(range/2),(range/2)),ylab="Depth [dm]",xlab="dRestivity [Ohmm]",type="l")
##negative only
plot(NA,NA,ylim=c(depthMax,0),xlim=c(-(range),0),ylab="Depth [dm]",xlab="dResistivity [Ohm m]",type="l")
abline(v=c(0))
...
for (i in seq(startRaster,endRaster,by=1)){
  ...
  lines(data.mean$ratio+data.sd[,2],data.mean$depth,col=colors[i-startRaster],lty=2)
  lines(data.mean$ratio-data.sd[,2],data.mean$depth,col=colors[i-startRaster],lty=2)
}
...
matplot(x=as.POSIXct(ert.time[,1],origin="1970-01-01"),as.data.frame(t(ert.depth[,-1]))[1:10],type="l",ylab="Resistivity difference [Ohm m]",xlab="Time",ylim=c(roundDown(min(ert.depth[,-1]),1),0),col=colors.depth,lty=1,lwd=2,axes=F)
...
##plot tdr data
##############################################################################
setwd("C:\\PHD\\Publications\\9920_infiltration\\53_tdr")
...
##filter data.frame
filterWindow=1200 ##seconds!!
movingAverage=function(x,n){filter(x,rep(1/n,n),sides=2)}
...
axis(side=2,at=seq(maxDepth,minDepth,by=-increment),labels=seq(minDepth/100,maxDepth/100,by=increment/100))
...
for (depth in depths$depth){
  ...
  plot(depthData$seconds,depthData$resDiff,type="p",ylim=c(-40,0),ylab="Resistivity difference [Ohm m]",xlab="Time [s]")
...
##FIT SWCC
#################################################################################################
setwd("C:\\PHD\\Code_Projects\\55_check_ert\\08_vanGenuchten\\curve_fitting")
