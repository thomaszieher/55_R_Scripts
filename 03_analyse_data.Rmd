...
reldiff=(rangeERT/material.before)*100
plot(reldiff,axes=F,box=F,add=T,asp=1,zlim=c(0,4.5))

boxplot(na.omit(reldiff@data@values),ylim=c(0,max(na.omit(reldiff@data@values))))
```
...
execGRASS("r.series",input=rlist[2:16],output="total_range",method="range",flag="overwrite")
...
<<<<<<< HEAD
uncertainty=3
relRange@data@values[relRange@data@values<=uncertainty]=0
=======
##omit values lower than 3% (uncertainty)
uncertainty=3
relRange@data@values[relRange@data@values<uncetainty]=0
>>>>>>> f03100fd76304ba07d90e06d02f94874448bbd29
...
plot(relRange,axes=F,box=F,add=T,asp=1,zlim=c(3,25))
points(dcpt.vect,pch=20,cex=0.5)
points(probesERT,pch=3,cex=0.5)
...
for (i in seq(startRaster,endRaster,by=1)){
  ...
  ##check distribution of resistivity ratios
  #hist(data[,1][data$depth==1])
  #shapiro.test(data[,1][data$depth==7])
  
  names(data.mean)=c("depth","ratio","depth2")
  names(data.sd)=c("depth","sd","depth2")
  ##add to time data frame
  data.append=data.frame(data.mean$ratio)
  sd.append=data.frame(data.sd$sd)
  names(data.append)=c(sprintf("tstep_%i",i))
  names(sd.append)=c(sprintf("tstep_%i",i))
  ert.depth=cbind(ert.depth,data.append)
  sd.depth=cbind(sd.depth,sd.append)
...
##plot uncertainty as raster
###Plot water content with depth over time  
##build matrix
sdData=as.matrix(sd.depth[,-1])

##rasterize matrix
sdDatarast=raster(sdData)



par(mfrow=c(1,1))
##plot uncertainty (stabw)
colorsUncert=colorRampPalette(c("white","red"))(30)
par(xaxs="i",yaxs="i",mar=c(4,5,4,6))
plot(NA,NA,xlim=c(0,1),ylim=c(0,1),axes=F,ylab="",xlab="")
lag=1/(endRaster-1)/2
axis(side=1,at=seq(lag,1-lag,length.out=(endRaster-1)),labels=format(as.POSIXct(ert.time[,1],origin="1970-01-01"),format="%H:%M"),las=2)
axis(side=2,at=seq(0,1,0.2),labels=seq(100,0,-20),las=1)
mtext(side=1,"Time",line=2.5)
mtext(side=2,"Depth [m]",line=2.5,las=0)
mtext(side=4,line=4,"Standard deviation [%]",las=0)
plot(sdDatarast,col=colorsUncert,add=T,zlim=c(0,5.3))
```
...
for (depth in depths$depth){
  ...
  lines(fitData$seconds,fitData$resist,col="red")
    ...
    ##plot times of tdr measurements
  #abline(v=dataProbes$time_16,col="red")
  ##tdr data at depth
  dataTDR.sub=dataTDR[dataTDR$probe==probes$nr[probes$depth==depth],]
  
  par(new=T)
  plot(dataTDR.sub$date,dataTDR.sub$vol,xlim=c(min(depthData$date),max(depthData$date)),pch=3,ylim=c(32,42),axes=F,ylab="",xlab="")
  axis(side=4,at=seq(32,42,2))
  mtext(side=4,"Soil water content",line=2,las=0)
  ...
  plot(NA,NA,xlim=c(32,42),ylim=c(-10,0),ylab="Resistivity ratio [%]",xlab="Soil moisture [Vol%]",las=1)
...
waterDepth=data.frame(watercontent1=vanGenuchten(-ciSuction[,1][1:101]*100,coef(genuchtenFit)[1],coef(genuchtenFit)[2],coef(genuchtenFit)[3],coef(genuchtenFit)[4])*100,watercontent2=vanGenuchten(-ciSuction2[,1][1:101]*100,coef(genuchtenFit)[1],coef(genuchtenFit)[2],coef(genuchtenFit)[3],coef(genuchtenFit)[4])*100,depth=depth[1:101]*100)
...
##soil water content for linear suction
...
plot(NA,NA,xlim=c(0,1),ylim=c(0,1),axes=F,ylab="",xlab="")
lag=1/(endRaster-1)/2
axis(side=1,at=seq(lag,1-lag,length.out=(endRaster-1)),labels=format(as.POSIXct(ert.time[,1],origin="1970-01-01"),format="%H:%M"))
...
##time
time=c(0,420,820,1260,1680,2100,2520,2940,3360,3780,4200,4620,5040,5460,5880,6300)
#time=c(0,0,0,420,820,1260,1680,2100,2520,2940,3360,3780,4200,4620,5040,5460,5880,6300,6720)

##precipitation (27.5 mm/h)
iz=c(0.0,0.0,0.0,0.0,0.00000763889,0.00000763889,0.00000763889,0.00000763889,0.00000763889,0.00000763889,0.00000763889,0.00000763889,0.00000763889,0.00000763889,0.00000763889)
...
##extent excluding initial conditions
exclude=5
ext=list(x=seq(exclude*(1/(endRaster-1)),1,length.out=(endRaster-1)), y=seq(0,1,length.out=nrow(data.r)))
extent(ext)
obs=crop(Suctionrast,ext)
...
#     }
    ...
    ##compute correlation coefficient for whole data set
    raststack=stack(data.r,Suctionrast)
    corel=layerStats(raststack,'pearson')
    
    ##exclude initial conditions for correlation (first five measurements)
    #model=crop(data.r,ext)
    #raststack=stack(obs,model)
    #corel=layerStats(raststack,'pearson')
    #corel$'pearson correlation coefficient'[1,2]
    
    
    #cor.test(data.r@data@values,Suctionrast@data@values)
    ttest=t.test(data.r@data@values,Suctionrast@data@values,paired=T)
    
    corelList=rbind(corelList,data.frame(ks=ks,ss=ss,d0=d0,cor=corel$"pearson correlation coefficient"[1,2],ttest=ttest$p.value))
  }
}
...
wireframe(cor~log10(ss)*log10(ks),
          ...
          xlab=list("LOG10 Specific storage [1/m]",rot=19),
          ylab=list("LOG10 Hydraulic conductivity [m/s]",rot=300),
          scales = list(arrows = FALSE))


best=corelList[corelList$cor==max(corelList$cor),]
best2=corelList[corelList$ttest==max(corelList$ttest),]

#plot(corelList$cor,corelList$ttest)
```


###Compare parameters
```{r echo=F,message=F,warning=F}
##parameter boxplot for correlation >=0.90
boxplot(log10(corelList[corelList$cor>=0.90,]$ks),
        log10(corelList[corelList$cor>=0.90,]$ss),
        log10(corelList[corelList$cor>=0.90,]$d0),
        names=c("Kz","Ss","D0"),
        ylim=c(-8,-1))

nrow(corelList[corelList$cor>=0.90,])
...
##check correlation
shapiro.test(log10(abs(data.r@data@values)))
shapiro.test(log10(abs(Suctionrast@data@values)))

##standard error of the mean
se=function(x){
  sd(x)/(length(x))^0.5
}
##confidence interval of the mean for student-t-distribution
cint=function(x,alpha){
  qt(1-alpha/2,df=length(x)-1)*se(x)
}
plot(data.r@data@values,Suctionrast@data@values)
t.test(data.r@data@values,Suctionrast@data@values,paired=T)

cintobs=cint(Suctionrast@data@values,0.05)
cintmodel=cint(data.r@data@values,0.05)
mean(data.r@data@values)
mean(Suctionrast@data@values)

##r squared
datafit=data.frame(model=data.r@data@values,obs=Suctionrast@data@values)
plot(datafit$obs,datafit$model,xlim=c(-30,-5),asp=1,xlab="Observations",ylab="Model")
devFit=lm(model~obs,data=datafit)
regr=data.frame(x=seq(-30,0,1))
regr["y"]=predict(devFit,newdata=data.frame(obs=regr$x))
lines(regr$x,regr$y,col="red")
text(-10,-27,sprintf("Rsq=%0.2f",summary(devFit)$r.squared),cex=0.8)
summary(devFit)
...
colorsreldiff=colorRampPalette(c("red","white","#9cd01b"))(100)
par(mfrow=c(1,1))
##plot absolute change
par(xaxs="i",yaxs="i",mar=c(4,5,4,6))
plot(NA,NA,xlim=c(0,1),ylim=c(0,1),axes=F,ylab="",xlab="")
<<<<<<< HEAD
axis(side=1,at=seq(1:ncol(scaleRatio))/16-(1/32),labels=format(as.POSIXct(ert.time[,1],origin="1970-01-01"),format="%H:%M"))
=======
lag=1/(endRaster-1)/2
axis(side=1,at=seq(lag,1-lag,length.out=(endRaster-1)),labels=format(as.POSIXct(ert.time[,1],origin="1970-01-01"),format="%H:%M"))
>>>>>>> f03100fd76304ba07d90e06d02f94874448bbd29
...
<<<<<<< HEAD
plot(diffrast,col=colors,add=T,zlim=c(-0.45,0.45))

diffrast=(RatioDatarast-data.r)
plot(diffrast,col=colors,add=T,zlim=c(-5,5))

matrix(data.r@data@values,nrow=10)
matrix(data.r@data@values,nrow=10)
...
color.bar(colorRampPalette(c("red","white","#9cd01b"))(21), -0.4)

=======
#plot(diffrast,col=rev(colors)[(10/7):(length(colors))],add=T,zlim=c(-0.15,0.45))
plot(diffrast,col=colorsreldiff,add=T,zlim=c(-0.42,0.42))
>>>>>>> f03100fd76304ba07d90e06d02f94874448bbd29
